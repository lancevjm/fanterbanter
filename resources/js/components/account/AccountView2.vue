                        ...
                        <table class="table align-items-center mb-0">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">View Statement</th>
                                </tr>
                            </thead>
                            <tbody>
                                    ...
                                    <td>
                                        <div class="d-flex px-2 py-1">
                                            <div class="d-flex flex-column justify-content-center">
                                                <h6 class="mb-0 text-sm">{{ customer.userprofile.user.name }} {{ customer.userprofile.user.surname }}</h6>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <p class="text-sm text-secondary mb-0">{{customer.account_number}}</p>
                                    </td>
                                    <td>
                                        <p class="text-sm text-secondary mb-0">{{customer.current}}</p>
                                    </td>
                                    <td>
                                        <span class="badge badge-dot me-4">
                                            <i class="bg-info" v-show="!arrears(customer.day_30)"></i>
                                            <i class="bg-danger" v-show="arrears(customer.day_30)"></i>
                                            <span class="text-dark">{{customer.day_30}}</span>
                                        </span>
                                    </td>
                                    <td class="align-middle text-center text-sm">
                                        <span class="badge badge-dot me-4">
                                            <i class="bg-info" v-show="!arrears(customer.day_60)"></i>
                                            <i class="bg-danger" v-show="arrears(customer.day_60)"></i>
                                            <span class="text-dark">{{customer.day_60}}</span>
                                        </span>
                                    </td>
                                    <td class="align-middle text-center text-sm">
                                        <span class="badge badge-dot me-4">
                                            <i class="bg-info" v-show="!arrears(customer.day_90)"></i>
                                            <i class="bg-danger" v-show="arrears(customer.day_90)"></i>
                                            <span class="text-dark">{{customer.day_90}}</span>
                                        </span>
                                    </td>
                                    <td class="align-middle text-center text-sm">
                                        <span class="badge badge-dot me-4">
                                            <i class="bg-info" v-show="!checkBalance(customer.balance, customer.current)"></i>
                                            <i class="bg-danger" v-show="checkBalance(customer.balance, customer.current)"></i>
                                            <span class="text-dark">{{ customer.balance }}</span>
                                        </span>
                                    </td>
